...
builder.Services.AddMassTransit(config =>
{
    ...
    config.UsingRabbitMq((context, cfg) =>
    {
        ...
        cfg.ReceiveEndpoint("http-request-created-event",
            endpointConfig => { endpointConfig.ConfigureConsumer<HttpRequestCreatedConsumer>(context); });
    });
});

// Configure the storage service
builder.Services.AddSingleton<IStorageService<HttpRequestCreated>, FileStorageService>(serviceProvider =>
{
    ILogger<FileStorageService> logger = serviceProvider.GetRequiredService<ILogger<FileStorageService>>();
