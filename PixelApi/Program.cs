...
builder.Services.AddHealthChecks()
    .AddRabbitMQ(
        new Uri(builder.Configuration.GetConnectionString("RabbitMq") ?? throw new InvalidOperationException()));
...
app.MapGet("/Track", async (HttpContext context, IPublishEndpoint endpoint, ILogger<Program> logger) =>
{
    byte[] trackingPixelData = Convert.FromBase64String("R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==");
    ...
    try
    {
        ...
        logger.LogDebug(
            ...
            referer, userAgent, ipAddress);

        var requestCreatedEvent = new HttpRequestCreated(referer, userAgent, ipAddress, Guid.NewGuid().ToString(),
            context.Items["RequestDate"] as DateTime? ?? DateTime.UtcNow);
